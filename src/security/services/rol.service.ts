...
export class RolService {
  constructor(
    ...
    private readonly opcionRepository: Repository<Rol>
  ) {}

  async findAll(): Promise<StatusResponseDto<any>> {
    try {
      const opciones = await this.opcionRepository.find({ relations: ['modulo'] });
      return new StatusResponseDto(true, 200, 'Roles obtenidas', opciones);
    } catch (error) {
  ...
  async create(dto: CreateUpdateRolDto): Promise<StatusResponseDto<any>> {
    try {
  ...
  async update(id: number, dto: CreateUpdateRolDto): Promise<StatusResponseDto<any>> {
    try {
