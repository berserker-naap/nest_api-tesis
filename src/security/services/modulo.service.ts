...
export class ModuloService {
  constructor(
    ...
    private readonly opcionRepository: Repository<Modulo>
  ) {}

  async findAll(): Promise<StatusResponseDto<any>> {
    try {
      const opciones = await this.opcionRepository.find({ relations: ['modulo'] });
      return new StatusResponseDto(true, 200, 'Moduloes obtenidas', opciones);
    } catch (error) {
  ...
  async create(dto: CreateUpdateModuloDto): Promise<StatusResponseDto<any>> {
    try {
  ...
  async update(id: number, dto: CreateUpdateModuloDto): Promise<StatusResponseDto<any>> {
    try {
  ...
  async deleteMany(ids: number[]): Promise<StatusResponseDto<any>> {
    try {
      ...
      return new StatusResponseDto(true, 200, 'Moduloes eliminadas', ids);
    } catch (error) {
